...
public class PlayerController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        PlayStartCutSceneOfCurrentLevel(LevelManagerScript.instance.activeModeLevels[currentLevel], false);
    ...
    public IEnumerator LevelStart()
    {
        ...
        PlayStartCutSceneOfCurrentLevel(LevelManagerScript.instance.activeModeLevels[currentLevel], true);
        yield return new WaitForSeconds(LevelManagerScript.instance.activeModeLevels[currentLevel].startCustscene.cutsceneTime);
        if (LevelManagerScript.instance.activeModeLevels[currentLevel].startCustscene.loadAnimal)
        {
            AnimalPositionHolder.instance.SpawnAnimals(LevelManagerScript.instance.activeModeLevels[currentLevel].startCustscene.animals);
        }
        foreach (var level in LevelManagerScript.instance.activeModeLevels[currentLevel].directions)
        {
        level.direction.SetActive(false);
        }
        LevelManagerScript.instance.activeModeLevels[currentLevel].directions[0].direction.SetActive(true);
        rccCanvas.SetActive(true);
            RCC.StartEngine();
        rccCamera.SetActive(true);
        PlayStartCutSceneOfCurrentLevel(LevelManagerScript.instance.activeModeLevels[currentLevel], false);
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.CompareTag("LastParking"))
        {
            ...
            if (!cutsceneStarted)
            {
                ...
                StartCoroutine(StartCutscene(LevelManagerScript.instance.activeModeLevels[currentLevel].endCustscene,true));
            }
        }
        ...
        if(other.gameObject.CompareTag("MidParking"))
        {
            ...
            if (!cutsceneStarted)
            {
                ...
                foreach (var item in LevelManagerScript.instance.activeModeLevels[currentLevel].directions)
                {
    ...
    public IEnumerator StartCutscene(LevelManagerScript.Cutscenes cutscene,bool Last)
    {
        ...
        else
        {
            ...
            LevelManagerScript.instance.activeModeLevels[currentLevel].directions[activeDirectionNumber-1].direction.SetActive(false);
            LevelManagerScript.instance.activeModeLevels[currentLevel].directions[activeDirectionNumber].direction.SetActive(true);
            if(cutscene.loadAnimal)
            {
                AnimalPositionHolder.instance.DestroyAnimals(); 
